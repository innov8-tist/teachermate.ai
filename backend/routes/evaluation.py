...
async def crop_pdf_section(crop_request: CropRequest):
    ...
    try:
        ...
        print(f"Fetching PDF from S3 for cropping: {file_key}")
        ...
        try:
            ...
            pdf_content = response['Body'].read()
            print(f"✅ Downloaded PDF from S3 for cropping, size: {len(pdf_content)} bytes")
        except ClientError as e:
            if e.response['Error']['Code'] == 'NoSuchKey':
                raise HTTPException(status_code=404, detail=f"PDF not found in S3: {pdf_id}")
        ...
        print(f"❌ Error in crop_pdf_section: {str(e)}")
        import traceback
        traceback.print_exc()
...
async def submit_student_answer(
    ...
    try:
        ...
        for i, img_file in enumerate(answer_images):
            ...
            # Upload to S3 (we'll use the same service but with a different path)
            s3_url = s3_service.upload_pdf_image(content, f"student-{roll_number}-q{question_id}", i + 1)
